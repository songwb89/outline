    ...
    <div class="upm-modal relative z-10" style="width: 560px;">
      ...
      <div class="upm-content">
        ...
        <div id="upm-profile-content" class="upm-tab-content active">
          ...
          <div class="upm-info-row">
            <div class="upm-info-item">
              <div class="upm-info-label">微信绑定</div>
              <div class="upm-info-value upm-bind-value">
                <span id="upm-wechat-status">未绑定</span>
                <button class="upm-change-pwd-btn" id="upm-wechat-action" onclick="handleWechatBindingAction()">绑定</button>
              </div>
            </div>
          </div>
...
function loadUserProfileData() {
  ...
  var wechatStatusEl = document.getElementById('upm-wechat-status');
  var wechatActionEl = document.getElementById('upm-wechat-action');
  if (wechatStatusEl) {
    wechatStatusEl.textContent = data.wechatBound ? '已绑定' : '未绑定';
  }
  if (wechatActionEl) {
    wechatActionEl.textContent = data.wechatBound ? '解绑' : '绑定';
  }
...
function saveCurrentTeacherData(data) {
  localStorage.setItem('currentTeacher', JSON.stringify(data));
}

function handleWechatBindingAction() {
  var data = getCurrentTeacherData();
  if (data.wechatBound) {
    showWechatUnbindModal();
  } else {
    showWechatBindModal();
  }
}

function showWechatBindModal() {
  var html = '<div class="upm-modal-overlay" id="upm-wechat-bind-overlay" onclick="if(event.target===this)hideWechatBindModal()">' +
    '<div class="upm-modal-content">' +
      '<div class="upm-modal-header">' +
        '<div class="upm-modal-title">绑定微信</div>' +
        '<button class="upm-modal-close" onclick="hideWechatBindModal()">&times;</button>' +
      '</div>' +
      '<div class="upm-modal-body" style="text-align:center;">' +
        '<img src="img/071f3kQV2ASf100M.jpg" alt="微信绑定二维码" onclick="submitWechatBind()" style="width:220px;height:220px;object-fit:cover;border:1px solid #f0f0f0;border-radius:8px;background:#fff;margin:0 auto;display:block;cursor:pointer;" />' +
        '<div style="font-size:13px;color:#666;margin-top:12px;">请使用微信扫一扫绑定</div>' +
      '</div>' +
    '</div>' +
  '</div>';
  document.body.insertAdjacentHTML('beforeend', html);
...
function hideWechatBindModal() {
  var overlay = document.getElementById('upm-wechat-bind-overlay');
  if (overlay) overlay.remove();
}

function submitWechatBind() {
  var data = getCurrentTeacherData();
  data.wechatBound = true;
  data.wechatNickname = data.wechatNickname || '微信用户';
  saveCurrentTeacherData(data);
  hideWechatBindModal();
  loadUserProfileData();
  upmToast('微信绑定成功', 'success');
}

function showWechatUnbindModal() {
  var html = '<div class="upm-modal-overlay" id="upm-wechat-unbind-overlay" onclick="if(event.target===this)hideWechatUnbindModal()">' +
    '<div class="upm-modal-content">' +
      '<div class="upm-modal-header">' +
        '<div class="upm-modal-title">解绑微信</div>' +
        '<button class="upm-modal-close" onclick="hideWechatUnbindModal()">&times;</button>' +
      '</div>' +
      '<div class="upm-modal-body">' +
        '<div style="font-size:13px;color:#666;margin-bottom:12px;line-height:1.6;">请输入当前账号密码，验证通过后解除微信绑定。</div>' +
        '<div class="upm-form-group">' +
          '<input type="password" id="upm-wechat-unbind-pwd" class="upm-form-input" placeholder="请输入登录密码" />' +
        '</div>' +
        '<div id="upm-wechat-unbind-error" style="color:#ff4444;font-size:12px;display:none;">请输入密码</div>' +
      '</div>' +
      '<div class="upm-modal-footer">' +
        '<button class="upm-btn upm-btn-cancel" onclick="hideWechatUnbindModal()">取消</button>' +
        '<button class="upm-btn upm-btn-primary" onclick="submitWechatUnbind()">确认解绑</button>' +
      '</div>' +
    '</div>' +
  '</div>';
  document.body.insertAdjacentHTML('beforeend', html);
...
function hideWechatUnbindModal() {
  var overlay = document.getElementById('upm-wechat-unbind-overlay');
  if (overlay) overlay.remove();
}

function submitWechatUnbind() {
  var pwdEl = document.getElementById('upm-wechat-unbind-pwd');
  var errorEl = document.getElementById('upm-wechat-unbind-error');
  ...
  var data = getCurrentTeacherData();
  data.wechatBound = false;
  data.wechatNickname = '';
  saveCurrentTeacherData(data);
  hideWechatUnbindModal();
  loadUserProfileData();
  upmToast('微信解绑成功', 'success');
}
